...
// Unity3D Plguin interfaces for windows
#ifdef _WIN32

static libusb_device_handle *win_handle;

int _declspec(dllexport) Win_UsbPort_open(uint16_t vid, uint16_t pid)
{
	libusb_device *dev;
	libusb_device_handle *handle;
	...
	if (status < 0) {
		printf("libusb_init failed. status = %d", status);
		return status;
	}

	handle = libusb_open_device_with_vid_pid(NULL, (uint16_t)vid, (uint16_t)pid);
	if (handle == NULL) {
		printf("libusb_open_device_with_vid_pid failed. vid = %d, pid = %d", vid, pid);
		return 0;
	}
	...
	if (status < 0) {
		printf("libusb_claim_interface failed. status = %d", status);
		return status;
	}
	win_handle = handle;
	return 1;
...
void _declspec(dllexport) Win_UsbPort_close()
{
	libusb_device_handle *handle = win_handle;
	libusb_release_interface(handle, 0);
	libusb_close(handle);
	libusb_exit(NULL);
}

int _declspec(dllexport) Win_UsbPort_write(unsigned char* data, int len)
{
	int actual;
	//my device's out endpoint was 1, the device had 2 endpoints: 1 and 129
	int r = libusb_bulk_transfer(win_handle, (1 | LIBUSB_ENDPOINT_OUT), data, len, &actual, 0);
	// 0: success
	if(r != 0)
		return r;
	return actual;
...
_declspec(dllexport) unsigned char* Win_UsbPort_read()
{
	int actual;
	libusb_device_handle *handle = win_handle;
	const int len = 61;
	...
	int r = libusb_bulk_transfer(handle, (129 | LIBUSB_ENDPOINT_IN), data, len, &actual, 0);
	return data;
}
/*Interfaces for windows end*/
#endif 


#ifdef __ANDROID__
// JNI
static libusb_device_handle *jni_handle;
JNIEXPORT jint JNICALL Java_android_1usb_1api_UsbPort_open(JNIEnv* env, jobject thiz, jchar vid, jchar pid)
{
	libusb_device *dev;
	libusb_device_handle *handle;
	struct libusb_config_descriptor *conf_desc;
  ...
  int status = libusb_init(NULL);
	if (status < 0) {
		LOGD("libusb_init failed. status = %d", status);
		return status;
	}
	
	handle = libusb_open_device_with_vid_pid(NULL, (uint16_t)vid, (uint16_t)pid);
	if (handle == NULL) {
		LOGD("libusb_open_device_with_vid_pid failed. vid = %d, pid = %d", vid, pid);
		return 0;
	}
	...
	status = libusb_claim_interface(handle, 0);
	if (status < 0) {
		LOGD("libusb_claim_interface failed. status = %d", status);
		return status;
	}
  jni_handle = handle;
  return 1;
...
JNIEXPORT void JNICALL Java_android_1usb_1api_UsbPort_close(JNIEnv* env, jobject thiz)
{
	libusb_device_handle *handle = jni_handle;
	libusb_release_interface(handle, 0);
	libusb_close(handle);
  libusb_exit(NULL);
}

JNIEXPORT jint JNICALL Java_android_1usb_1api_UsbPort_write(JNIEnv* env, jobject thiz, jbyteArray buffer)
{
	int actual;
  libusb_device_handle *handle = jni_handle;
  int len = (*env)->GetArrayLength(env, buffer);
  unsigned char *data = (unsigned char *)(*env)->GetByteArrayElements(env, buffer, 0);
  //my device's out endpoint was 1, the device had 2 endpoints: 1 and 129
	int r = libusb_bulk_transfer(handle, (1 | LIBUSB_ENDPOINT_OUT), data, len, &actual, 500);
...
JNIEXPORT jbyteArray JNICALL Java_android_1usb_1api_UsbPort_read(JNIEnv* env, jobject thiz)
{
  int actual;
  libusb_device_handle *handle = jni_handle;
  const int len = 61;
...
#if defined(OS_LINUX)
static libusb_device_handle *linux_handle;
int API_EXPORTED Linux_UsbPort_Open(uint16_t vid, uint16_t pid)
{
	libusb_device *dev;
	libusb_device_handle *handle;
	struct libusb_config_descriptor *conf_desc;
...
void API_EXPORTED Linux_UsbPort_Close()
{
	libusb_device_handle *handle = linux_handle;
	libusb_release_interface(handle, 0);
	libusb_close(handle);
	libusb_exit(NULL);
}

int API_EXPORTED Linux_UsbPort_Write(unsigned char* data, int len)
{
	int actual;
	//my device's out endpoint was 1, the device had 2 endpoints: 1 and 129
	int r = libusb_bulk_transfer(linux_handle, (1 | LIBUSB_ENDPOINT_OUT), data, len, &actual, 0);
	// 0: success
	if(r != 0)
		return r;
	return actual;
...
API_EXPORTED unsigned char* Linux_UsbPort_Read()
{
	int actual;
	libusb_device_handle *handle = linux_handle;
	const int len = 61;
...
void API_EXPORTED Linux_UsbPort_FreePtr(unsigned char* ptr)
{
	free(ptr);
}
